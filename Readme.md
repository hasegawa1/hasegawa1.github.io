...
タイトルを出力するためには次の命令を用いる。

|\title     |文書名の指定|
|\author    |著者名の指定。複数の場合は \and で区切る。|
|\date      |日付の指定。指定しない場合は処理した日付となる。|
|\maketitle |文書名・著者名・日付の出力|

タイトルの後に概要(Abstract)を書く場合は、 \maketitle の直後に \begin{abstract} と \end{abstract} で囲んで書く。
