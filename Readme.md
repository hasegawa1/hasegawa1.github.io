...
```
というプロンプトになっている。これを「カレントディレクトリ$」というプロンプトに変更するには、以下のコマンドを実行する。端末の制御文字についてはググって。
```
export PS1="\[\e]0;\t: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;36m\]\t\[\033[00m\]:\[\033[01;35m\]\w\[\033[00m\]\$ "
```
これを ~.bashrc に書けば設定が保持される。
```
echo export PS1="\[\e]0;\t: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;36m\]\t\[\033[00m\]:\[\033[01;35m\]\w\[\033[00m\]\$ " >>.bashrc
```
bashのプロンプトには、以下のような特殊な文字を使用できる。
